...
def check(filename, checksum, checksum_type):
    BUF_SIZE = 65536
    d = {"sha256": hashlib.sha256(), "sha1": hashlib.sha1(), "md5": hashlib.md5()}
...
def main():
    ...
    try:
        ...
        sys.exit(
            f"FATAL: Cannot create backup of Vagrant catalog metadata file \
            {args.catalog}: {err}"
        )
    ...
    try:
        ...
        sys.exit(
            f"FATAL: Cannot write to Vagrant catalog metadata file \
            {args.catalog}: {err}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Update Vagrant catalog metadata file")
    parser.add_argument(
        "-f",
        "--catalog",
        required=True,
    ...
    parser.add_argument("-v", "--version", type=str, help="New box version")
    parser.add_argument(
        "-p",
        "--provider",
        default="libvirt",
    ...
    parser.add_argument("-b", "--box", type=str, help="Path to Vagrant box file")
    parser.add_argument(
        "-t",
        "--checksum_type",
        default="sha256",
